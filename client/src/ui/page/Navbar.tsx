  ...
  return (
    <nav id={'navbar'} className='w-100'>
      <ul className={`${ulC} ${ulC2}`}>
        <div>
          <Link className={homeC} to="/"><li className='bg-transparent text-white p-2'>{t('home')}</li></Link>
          {token && role == 'admin' && <Link className={uC} to="/users"><li className={uC2}>{t('users')}</li></Link>}
        </div>

        <div className='d-flex flex-row justify-content-center align-items-center'>
          {token && <Link className={usernameC} to="/profile"><li className={usernameC2}>Username</li></Link>}
          {!token && <Link className={nusernameC} to="/login"><li className={nusernameC2}>{t('login')}</li></Link>}

          {token && <button onClick={handleLogout} className='btn btn-primary m-2 p-2'>{t('logout')}</button>}
          {!token && <Link className={signupC} to="/signup"><li className={signupC2}>{t('signup')}</li></Link>}
        </div>
      </ul>
    </nav>
  );
